...
subplot(2,1,1);
plot(kk*(sqrt(c2)/wps(1)),real_w(:,:)/abs(wcs(1)),'k.');
xlabel('ck/\omega_{pe}');
...
subplot(2,1,2);
plot(kk*(sqrt(c2)/wps(1)),imag_w(:,:)/abs(wcs(1)),'k.-');
xlabel('ck/\omega_{pe}');
...
h=figure('unit','normalized','Position',[0.01 0.45 0.5 0.6],'DefaultAxesFontSize',20);
subplot(2,1,1);
plot(kk*(sqrt(c2)/wps(1)),real_w(:,rootNo)/abs(wcs(1)),'k.','markersize',15);
xlabel('ck/\omega_{pe}');
ylabel('\omega_r/\omega_{ce}');
grid on;
xlim([0 3]);
...
subplot(2,1,2);
plot(kk*(sqrt(c2)/wps(1)),imag_w(:,rootNo)/abs(wcs(1)),'b.-','markersize',15);
xlabel('ck/\omega_{pe}');
ylabel('\gamma/\omega_{ce}');
grid on;
xlim([0 3]);
