...
%% for plot
close all;
h=figure('unit','normalized','Position',[0.01 0.45 0.65 0.4],...
...
%       ylabel('\omega_i/\omega_{r}'); box on; grid on;
    ...
    surf(ppa,ppb,real(wwjp)); hold on; box on; s
    et(gca,'BoxStyle','full');
    xlabel('\phi');
    ylabel('k\rho_{ce}');
    axis tight;
    view(3);
...
save('saveData_Gary1993Book_mirror_TpxTpz2dot25_th66_Ki4_Je8.mat','pas','wws');

% load('saveData_Cattaert2007Fig7_deg5.mat');
% plot(pas,imag(wws(:,1,1)))
...
print(gcf,'-depsc',['Gary1993Book_mirror_TpxTpz2dot25_th66_Ki4_Je8_','select.eps']);
savefig(['Gary1993Book_mirror_TpxTpz2dot25_th63_Ki4_Je8_','select.fig']);
