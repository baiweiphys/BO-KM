...
subplot(2,1,1);
plot(kk*rhocs(1),real_w(:,:)/abs(wcs(1)),'k.','markersize',10);
grid on;
xlabel('k\rho_{ce}');
...
subplot(2,1,2);
plot(kk*rhocs(1),imag_w(:,:)/abs(wcs(1)),'b.-','markersize',10);
grid on;
xlabel('k\rho_{ce}');
...
%% plot the selected value of growth rate
rootNo = 2:40;
...
subplot(2,1,2);
plot(kk*rhocs(1),imag_w(:,rootNo)/abs(wcs(1)),'b.-','markersize',15);
grid on;
xlabel('k\rho_{ce}');
ylabel('\gamma/\omega_{ce}');
xlim([0,1]);
