...
%       ylabel('\omega_i/\omega_{r}'); box on; grid on;
    ...
    xlabel('\phi[\circ]');
    ylabel('k\rho_{cp}');
    zlabel('\omega_r/｜\omega_{cp}｜');
        axis tight;
    view(3);
    % title(['(a) \beta_{||}=',num2str(betasz,3),...
    %   ', \beta_\perp=',num2str(betasp,3)]);
    subplot(122);
    s = surfc(ppa,ppb,imag(wwjp)); 
    % s(2).Fill = 'on';
    s(2).LineWidth = 2;
    %
    hold on; box on;
...
save('Case_EMIC_allAngle_B0=100nT_Tsz=1879.mat','ppa','ppb','wws');
set(gcf,'Units','inches');
screenposition = get(gcf,'Position');
set(gcf,'PaperPosition',[0 0 screenposition(3:4)],...
  'PaperSize',[screenposition(3:4)]);

print(gcf,'-depsc',['Case_EMIC_allAngle_B0=100nT_Tsz=1879','_select.eps']);
savefig(['Case_EMIC_allAngle_B0=100nT_Tsz=1879','_select.fig']);

%%
% figure;
% subplot(1,2,1);
% plot(pb,real(wwjp(12,:)))
% ylim([0 1]);
% 
% subplot(1,2,2);
% plot(pb,imag(wwjp(12,:)))
% ylim([-0.04,0.04]);
