...
save(fname,'theta','kk0','kk','kk','ww','wps','wcs','lambdaDs','rhocs','N','betas_para','betas_perp','runtime');
...
subplot(2,1,1)
for ii=1:10
    contourf(KKX,KKZ,real_w(:,:,ii)'/abs(wcs(1)),'LineColor','none');
    hold on;
...
subplot(2,1,2);
for ii=1:1
    contourf(KKX,KKZ,imag_w(:,:,ii)'/abs(wcs(1)),'LineColor','none');
    hold on;
end
shading interp
colormap('jet');
...
NN = 20;
for ii=1
    rww = reshape(real_w(:,:,ii)',num,1);
    rkk = reshape(KK*rhocs(1),num,1);
    rth = reshape(THETA*180/pi,num,1);
...
subplot(2,1,1);
hold on;
plot(kk*rhocs(1),real_w(iTH,:,rootNo)/abs(wcs(1)),'r.','markersize',5);
grid on;
xlabel('k\rho_{cp}');
ylabel('\omega_r/\omega_{cp}');
xlim([0 0.3]);
...
hold on;
plot(kk*rhocs(1),imag_w(iTH,:,rootNo)/abs(wcs(1)),'g.-','markersize',5);
grid on;
xlabel('k\rho_{cp}');
ylabel('\gamma/\omega_{cp}');
% xlim([0 0.3]);
